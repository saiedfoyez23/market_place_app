...
class PlannerMessageController extends GetxController {
  ...
  PlannerMessageController({required this.context});
  ...
  void onInit() {
    ...
    Future.delayed(Duration(microseconds: 100),() async {
      ...
      plannerSocketServiceController.socket!.on('chat-list::${userLoginResponseModel.value.data?.user?.sId}', (data) async {
  ...
  Future<void> seenMessageController({
    ...
    BaseApiUtils.patch(
      ...
      authorization: userLoginResponseModel.value.data?.accessToken,
      onSuccess: (e,data) async {
        Get.off(()=>PlannerChatView(chatId: chatId,),preventDuplicates: false);
